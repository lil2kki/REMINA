...
uniform float u_darkness = 0.24; // Контроль темноты (0.0 - 1.0)
uniform float u_desaturation = 0.8; // Контроль насыщенности (0.0 - 1.0)
...
vec3 addBloodStains(vec2 uv, float time) {
    ...
    return vec3(bloodPattern * 0.8, 0.1, 0.1) * pulse;
}
...
void main() {
    ...
    float blockSize = 10.0 + sin(u_time * 0.3) * 5.0;
    ...
    float strongGlitch = step(1.93, blockRand);
    float mediumGlitch = step(1.88, blockRand) * (1.0 - strongGlitch);
